...
import DH.DB.beans.BnUser;
import DH.DB.clases.Encriptar;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            while (nombresParametros.hasMoreElements()) {
                String key = (String) nombresParametros.nextElement();
                String val = request.getParameter(key);
                parametros.put(key, val);
            }

            BnUser bn = new BnUser();
            HttpSession session = request.getSession();
            switch (parametros.get("accion")) {
                case "login": {
                    JSONObject json = new JSONObject();
                    String user = parametros.get("username");
                    String clave = parametros.get("password");
                    //encriptar en el JS
                    Usuario credenciales = bn.iniciarSesion(user, Encriptar.toMD5(clave));
